import random
from automatic_prompt_engineer import generate, evaluate, config, template, data, llm, social_media_data
...
def simple_ape(history_data,
    ...
    Parameters:
        history_data: The data to use for prompt generation (history split).
        holdout_data: The data to use for evaluation (holdout split).
    ...
    return find_prompts(eval_template, demos_template, history_data, holdout_data, conf, prompt_gen_template=prompt_gen_template)


def simple_eval(holdout_data,
                prompts,
                eval_template='User persona: [INPUT]\nGenerate an original tweet: [PROMPT]\nTweet: [OUTPUT]',
                demos_template='User persona: [INPUT]\nGenerate an original tweet: [OUTPUT]',
                eval_model='gpt-4o',
    ...
    conf['evaluation']['num_samples'] = min(len(holdout_data[0]), num_samples)
    res = evaluate.evalute_prompts(
        prompts, eval_template, holdout_data, demos_template, holdout_data, conf['evaluation']['method'], conf['evaluation'])
    return res


def simple_estimate_cost(history_data,
                         holdout_data,
                         eval_template='User persona: [INPUT]\nGenerate an original tweet: [PROMPT]\nTweet: [OUTPUT]',
                         prompt_gen_template=None,
                         demos_template='User persona: [INPUT]\nGenerate an original tweet: [OUTPUT]',
    ...
    return estimate_cost(eval_template, demos_template, history_data, holdout_data, conf, prompt_gen_template=prompt_gen_template)
...
def estimate_cost(eval_template,
    ...
    for query in queries:
        # Assuming query is the input text, and max_prompt_len is the expected output tokens
        query_cost += llm.gpt_get_estimated_cost(
            conf['generation']['model'], len(query) // 4, max_prompt_len) # Rough token estimation
    ...
    query_cost = 0
    for query in queries:
        # For evaluation, output tokens are typically 0 as we are getting logprobs or comparing
        # However, since logprobs are commented out, we'll assume minimal output tokens for cost estimation.
        # The actual output (generated tweet) is what we'll compare against.
        # For now, setting output_tokens to a small value or 0 if not generating.
        query_cost += llm.gpt_get_estimated_cost(model_name, len(query) // 4, 10) # Assuming 10 output tokens for evaluation query
